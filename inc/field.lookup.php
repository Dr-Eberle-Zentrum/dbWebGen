		...
		//--------------------------------------------------------------------------------------		
			...
			while($obj = $stmt->fetch(PDO::FETCH_OBJ)) {
				...
				$output_buf .= sprintf(
					...
					unquote($obj->val),
					$sel,
					format_lookup_item_label($obj->txt, $this->get_lookup_table_name(), $this->get_lookup_field_name(), $obj->val)
				);
			}					
			$output_buf .= "</select>\n";			
		}
				...
				while($obj = $res->fetchObject()) {						
					if(in_array("{$obj->val}", $linked_items)) {
						...
						$output_buf .= sprintf(
							...
							$obj->val, unquote($obj->txt), format_lookup_item_label($obj->txt, $this->get_lookup_table_name(), $this->get_lookup_field_name(), $obj->val)
						);
					}
				}
			}
