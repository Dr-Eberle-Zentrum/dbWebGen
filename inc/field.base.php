<?
	...
	//==========================================================================================
	{
		...
		//--------------------------------------------------------------------------------------		
			...
			switch($field['type']) {
				...
				case T_TEXT_LINE: case T_POSTGIS_GEOM: return new TextLineField($table_name, $field_name, $field);
		...
		//------------------------------------------------------------------------------------------
			...
			if($this->allow_setnull_box()) {							
				...
				$output_buf .= sprintf( 
					...
					"<input name='%s__null__' type='checkbox' value='true' %s />%s</label></div>",
					$visibility, $this->get_control_name(), $this->get_control_name(), $checked_attr, $this->get_setnull_label()
				);
			}
			return $output_buf;
		}
	...
	//==========================================================================================
	abstract class SingleLineTextInputField extends Field
	//==========================================================================================
	{
		//------------------------------------------------------------------------------------------
		...
		//--------------------------------------------------------------------------------------
			return $this->has_char_length() ? sprintf("maxlen='%s'", $this->get_char_length()) : '';
		}
